...
//---------------------------------------------------------------------------
/**************** Resolve partículas em zonas de fronteira *****************/
//---------------------------------------------------------------------------
void zonasFronteira(){
  //Achar o MAX e MÍN de cada quadro
  /*if(particula->x < menorparticulaX || particula->x > maiorparticulaX ||
              particula->y < menorparticulaY || particula->y > menorparticulaY ||
              particula->z < menorparticulaZ || particula->z > maiorparticulaZ) {
          adiciona a particula na lista de fronteiras;
      }*/
}
...
/******************************** FoF para um quadro ***********************/
//---------------------------------------------------------------------------
int quadrosFriends(particulas **first, float rperc){
  ...
  particulas *igru = malloc(sizeof(particulas)*(*first)->size);
  particulas *j_value = malloc(sizeof(particulas));
  particulas *l_value = malloc(sizeof(particulas));
  igru = *first;
...
//---------------------------------------------------------------------------
/******************** Adiciona uma partícula em um quadro ******************/
//---------------------------------------------------------------------------
void addParticula (particulas **first, int i, float x, float y, float z) {
    ...
    particulas *nova = malloc(sizeof(particulas));
...
/**************************** Limpa a memória ******************************/
//---------------------------------------------------------------------------
void liberaParticulas(particulas **first, int s){
  int i;
  for(i=0; i<s; i++){
    particulas *t = *first;
    *first = t->prox;
...
//---------------------------------------------------------------------------
/****************** Distribui as partículas para os quadros ****************/
//---------------------------------------------------------------------------
void divideEntrada(){
  ...
  for(int k = 0; k<8; k++){
    totalQuadros[k] = malloc(sizeof(particulas)*100);
    totalQuadros[k]->size = 0;
    totalQuadros[k]->grupo = 0;
  }

  //Distribui as partículas
  for(int i = 0; i < N; i++) {
      quadro = 0;
      if (x[i] > meioX)
          quadro += 1;
  ...
  //Verifica o total de grupos encontrados nos quadros e libera memória
  for(int k = 0; k<8; k++){
    total_grupos += quadrosFriends(&totalQuadros[k], 0.1);
    liberaParticulas(&totalQuadros[k], totalQuadros[k]->size);
  }
  printf("\nTotal de grupos:%d\n", total_grupos);
}
